      ...
      <form [formGroup]="form" *ngIf="isLoaded()" [ngSwitch]="isFlAttoInterruttivoAnzianita()">
        <a class="it-has-checkbox flex-column">
          <div class="it-right-zone w-100 border-bottom-0">
            <label class="text-dark">{{'label.jconon_application.fl_atto_interruttivo_anzianita' | translate }}</label>
            <div class="toggles mr-1">
                <label for="fl_atto_interruttivo_anzianita">
                    <input type="checkbox" (change)="onChangeFlAttoInterruttivoAnzianita(true)" id="fl_atto_interruttivo_anzianita" 
...
export class JcononAspectAttoInterruttivoAnzianitaComponent extends DynamicComponent {
    ...
    ngOnInit(): void {
      ...
      this.onChangeFlAttoInterruttivoAnzianita(false);
      super.ngOnInit();
    }

    public onChangeFlAttoInterruttivoAnzianita(reset: boolean) {
      if (reset) {
